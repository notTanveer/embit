"""
Tests for PSBTSigner with Silent Payment outputs.
...
Covers:
  ...
  - PSBTSigner discards incoming SP fields before repopulating.
  - PSBT version=2 is preserved after PSBTSigner.sign().
...
class TestE2EP2WPKHFromVector(unittest.TestCase):
    ...
    def test_dleq_proof_valid(self):
        """DLEQ proof generated by PSBTSigner.sign() verifies correctly."""
        if self.vector is None:
            self.skipTest("Test vector not found in bip375_test_vectors.json")

        psbt, priv = self._build_psbt()
        PSBTSigner(psbt).sign(priv)
    ...
    def test_full_psbt_sign_produces_signature(self):
        ...
        psbt, priv = self._build_psbt()
        count = PSBTSigner(psbt).sign(priv)
...
class TestPSBTSignerRejectsIneligibleInputs(unittest.TestCase):
    """PSBTSigner.sign() must raise SPValidationError for prohibited input types."""
    ...
    def test_multisig_p2sh_input_with_sp_output_no_sp_fields(self):
        ...
        PSBTSigner.sign() should complete without adding SP fields for that input.
        """
        root = _root()
...
# ── PSBTSigner behaviour tests ─────────────────────────────────────────────────


class TestPSBTSignerBehaviour(unittest.TestCase):
    ...
    def test_discards_incoming_sp_fields(self):
        ...
        PSBTSigner(self.psbt).sign(self.root)
    ...
    def test_version_preserved(self):
        """PSBTSigner.sign() does not downgrade the PSBT version to 0."""
        PSBTSigner(self.psbt).sign(self.root)
    ...
    def test_sp_fields_survive_serialization(self):
        """SP fields written by PSBTSigner survive a PSBT serialize/parse round-trip."""
        PSBTSigner(self.psbt).sign(self.root)
