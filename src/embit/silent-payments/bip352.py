...
def decode_silent_payment_address(address: str) -> Tuple[ec.PublicKey, ec.PublicKey]:
    ...
    try:
        witver, decoded = bech32.decode(hrp, address)
    except bech32.Bech32DecodeError as e:
        raise ValueError(f"Invalid silent payment address: {e}")

    if witver != 0:
        raise ValueError(
            f"Invalid silent payment address: unsupported version {witver}"
        )
    if len(decoded) != 66:
        raise ValueError(
            f"Invalid silent payment address: expected 66 bytes, got {len(decoded)}"
        )
